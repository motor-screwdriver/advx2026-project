...
/** Artifact-derived scoring modifiers applied before the score is computed. */
export interface ScoringModifiers {
  /** warm_blanket: skip the oversleep penalty entirely. */
  skipOversleepPenalty?: boolean
  /** coffee_amulet: extra forgiveness (minutes) for bedtime deviation. */
  bedTolerance?: number
  /** alarm_bell: extra forgiveness (minutes) for wake-time deviation. */
  wakeTolerance?: number
}
...
export function evaluateNight(
  ...
  const bedDev = Math.max(0, Math.abs(bedTime - window.bedMin) - (modifiers.bedTolerance ?? 0))
  const wakeDev = Math.max(0, Math.abs(wakeTime - window.wakeMin) - (modifiers.wakeTolerance ?? 0))
  const bedPenalty = deviationPenalty(bedDev)
  const wakePenalty = deviationPenalty(wakeDev)
  const shortfallPenalty = Math.min(Math.max(0, windowMin - duration), SHORTFALL_CAP_MIN) * 0.25
  const overslept = !modifiers.skipOversleepPenalty && duration > windowMin + OVERSLEEP_GRACE_MIN
  const score = clamp(
