import { applyNightResult, assignHero, evaluateNight, mockGameState } from '../mock'
...
describe('applyNightResult', () => {
  ...
  it('levels up on crossing a 700-XP boundary', () => {
    const base = stateWith('knight', 6)
    const state = { ...base, hero: { ...base.hero!, xp: 650 } }
    const next = applyNightResult(state, evaluateNight(state, 690, 1140), date)
    expect(next.hero!.level).toBe(state.hero!.level + 1)
    expect(next.hero!.xp).toBe(750)
  })

  it('resets the streak after 7 nights without HP loss (no level-up)', () => {
    const state = { ...stateWith('knight', 6), perfectWeekStreak: 6 }
    const next = applyNightResult(state, evaluateNight(state, 690, 1140), date)
    expect(next.hero!.level).toBe(state.hero!.level)
    expect(next.perfectWeekStreak).toBe(0)
  })
