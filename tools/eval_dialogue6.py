...
- 1-3 short sentences, then exactly one question. The message must contain exactly one question mark; rhetorical questions count too.
...
- Reply in English only, printable ASCII only: straight quotes, plain hyphens, no curly quotes, no em-dashes, no emoji.
...
- CRITICAL: the game renders the clock times itself, so message and reason must never reference
  clock times or durations in ANY form - no digits, no spelled-out numbers (not even "eleven" or
  "seven"), and none of the words: hour, hours, am, pm, midnight, noon, o'clock, tonight, morning
...
SCHEMA = {"type": "object", "properties": {"bedTime": {"type": ["string", "null"]}, "wakeTime": {"type": ["string", "null"]}, "message": {"type": "string"}, "suggestions": {"type": "array", "items": {"type": "string"}, "maxItems": 3}, "reason": {"type": ["string", "null"]}}, "required": ["bedTime", "wakeTime", "message", "suggestions", "reason"], "additionalProperties": False}
